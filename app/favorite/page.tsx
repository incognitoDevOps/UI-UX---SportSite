...
import { FavouritesConfig } from "@/config/favourites";
  ...
  return (
    <div className="px-2 rounded-lg" style={{ border: "solid 1px #8c8a8f" }}>
      ...
      >
        ...
        >
          {FavouritesConfig.name}
        </h1>
        
        <Calenda/>
      </div>

      <p className="text-sm text-justify py-2">{FavouritesConfig.description}</p>
      <div>
      <ScrollShadow hideScrollBar size={100} className="w-full h-[800px]">

        {FavouritesConfig.eventDetails.map((item) => (
          <div
            key={item.id}
